...
public class VehicleSelectionState extends WaitingState implements VehicleState
{
	...
	public VehicleSelectionState(StateGame stateGame, Arena arena)
	{
		...
		for(VehicleType type : new ArrayList<>(ready))
			if(((MobRacersSetup) stateGame.getSetup()).getVehicleLocations().get(type) == null)
				ready.remove(type);
	...
	public void giveStuff(Player player)
	{
		...
		saddleMeta.setDisplayName(CourseMessage.VEHICLESELECT_GUI_ITEM.toString());
		saddle.setItemMeta(saddleMeta);

		player.getInventory().setItem(0, saddle);
